...
import {Nav, NavItem, NavLink, InputGroup, Overlay, Popover} from "reactstrap";
        ...
        if (this.phase == 0) {
            ...
            for (let i = 0; i < fields.length; i++) {
                ...
                if (item.type == "string" || item.type == "number" || item.type == "date" || item.type == "select" || item.type == "radio") {
                    if (!filter || filter == "" || item.label.toLowerCase().match("^"+filter.toLowerCase())) {
                        nav.push(<NavItem><NavLink key={i+1} eventKey={i+1}
                                                   onClick={this.__onDecideKey.bind(undefined,i)}>{item.label}</NavLink></NavItem>);
                    }
                }
            }
            this.setState({
            ...
            for (let i = 0; i < this.operators.length; i++) {
                if (!filter || filter == "" || this.operators[i].toLowerCase().match("^"+filter.toLowerCase())) {
                    nav.push(<NavItem><NavLink key={i+1} eventKey={i+1}
                                               onClick={this.__onDecideKey.bind(undefined,i)}>{this.operators[i]}</NavLink></NavItem>);
                }
            }
            this.setState({
                nav: nav,
            ...
            if (this.filter.subject.type == "date") {
                ...
                for (let i = 0; i < items.length; i++) {
                    if (!filter || filter == "" || items[i].text.toLowerCase().match("^"+filter.toLowerCase())) {
                        nav.push(<NavItem><NavLink key={i+1} eventKey={i+1}
                                                   onClick={this.__onDecideKey.bind(undefined,i)}>{items[i].text}</NavLink></NavItem>);
                    }
                }
                this.setState({
                    nav: nav,
