...
import { Popover, PopoverContent, Button, ButtonGroup } from "reactstrap";
    ...
    __renderFilters(fields: Array<Map>, visiblePopups: Map): Array {
        ...
        for (let i = 0; i < fields.length; i++) {
            ...
            if (field.filter === true) {
                ...
                //     </Overlay>);
                filterField = (
                    <Popover placement="bottom" isOpen={show} target={colId} id="popover">
                        <PopoverContent>
                            <Filter field={field} value={this.state[field.name]} onChange={this.__onChange} delay={this.props.delay} />
                            <ButtonGroup size="sm" className="pull-right" style={{ marginBottom: "15px" }}>
                                <Button color="danger" onClick={onClear}>{this.props.clearButtonText}</Button>
                                <Button color="danger" onClick={this.__onClearAll}>{this.props.clearAllButtonText}</Button>
                            </ButtonGroup>
                        </PopoverContent>
                    </Popover>);
